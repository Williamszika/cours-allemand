...
function sysPrompt(niveau,langue,mode){const n=String(niveau||"A1").toUpperCase(),lg=String(langue||"fr");const deb=(n==="A1"||n==="A2");let s="Tu es un tuteur d'allemand bienveillant et patient. Niveau de l'apprenant: "+n+". Langue de l'apprenant (code): "+lg+". ";s+=deb?"Explique les regles et le vocabulaire DANS LA LANGUE DE L'APPRENANT, mais donne toujours les exemples en allemand. Phrases simples et courtes. ":"Reponds PRINCIPALEMENT EN ALLEMAND (immersion); clarifie brievement dans la langue de l'apprenant seulement si vraiment necessaire. ";s+="Corrige gentiment les fautes (montre la forme correcte + une courte explication). Encourage, reste concis (style conversation), et termine souvent par une petite question pour faire parler l'apprenant. ";if(String(mode||"")==="pflege")s+="CONTEXTE SPECIAL: allemand pour les soins infirmiers (Deutsch fuer die Pflege). Propose des jeux de role realistes (patient, collegue infirmier, medecin), enseigne le vocabulaire medical et les tournures professionnelles, et la facon de documenter/communiquer en milieu de soins. ";return s;}
...
http.createServer(async(rq,rs)=>{try{
  ...
  if(rq.method==="POST"&&/\/chat\/?$/.test(u.pathname)){
    ...
    rq.on("end",async()=>{try{
      ...
      const r=await anthropic(sysPrompt(body.niveau,body.langue,body.mode),msgs);
