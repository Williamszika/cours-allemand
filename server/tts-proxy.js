...
http.createServer(async(rq,rs)=>{try{
  ...
  if(rq.method==="POST"&&/\/admin\/?$/.test(u.pathname)){
    ...
    rq.on("end",function(){try{
      ...
      rs.writeHead(200,{"Content-Type":"application/json; charset=utf-8","Cache-Control":"no-store"});
      return rs.end(JSON.stringify({ok:true,isOwner:owner,ownerId:String(ADMIN_ID||""),admins:__admList,stats:{total:total,new7:new7,active7:act7,messages:msgs,byLevel:byLevel,byLang:byLang,topReferrers:topRef,competences:compStats(users)},users:rows}));
