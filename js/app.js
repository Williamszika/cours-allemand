...
(function () {
  ...
  /* Niveau (A1..C2) d'un module d'après son id ("a1-1" → "A1"). */
  function moduleNiveau(m) { return String((m && m.id) || "a1").split("-")[0].toUpperCase(); }
  /* Une dictée de module est-elle réussie ? (pas de dictée → considérée faite) */
  function dicteeFaite(modId) {
    if (!(window.DICTEES && window.DICTEES[modId])) return true;
    const ts = window.Progress.getTestScore("dictee-" + modId);
    return !!(ts && ts.reussi);
  }
  /* Toutes les dictées d'un niveau sont-elles réussies ? (verrou d'examen) */
  function dicteesFaites(code) {
    return (COURS.modules || []).filter((m) => moduleNiveau(m) === code).every((m) => dicteeFaite(m.id));
  }
  /* Prochaine étape DANS un niveau : leçon non faite, puis DICTÉE du module
     (OBLIGATOIRE) une fois toutes les leçons du module faites. null si tout est fait.
     `extraDone` = id d'une leçon à considérer comme déjà terminée (leçon en cours). */
  function etapeNiveau(code, extraDone) {
    for (const m of (COURS.modules || [])) {
      if (moduleNiveau(m) !== code) continue;
      for (const l of (m.lecons || [])) if (l.id !== extraDone && !window.Progress.estTermine(l.id)) return "#/lecon/" + l.id;
      if (!dicteeFaite(m.id)) return "#/dictee/" + m.id; // ✍️ dictée obligatoire avant la suite
    }
    return null;
  }
  /* Cœur du parcours gardé : leçon, dictée (obligatoire) ou examen → hash. */
  function prochaineEtapeCore(extraDone) {
    let e;
    if ((e = etapeNiveau("A1", extraDone))) return e;
    if (!examPasse("a1")) return "#/examen/a1";
    if ((e = etapeNiveau("A2", extraDone))) return e;
    if (!examPasse("a2")) return "#/examen/a2";
    if (!examPasse("final")) return "#/examen/final";
    if ((e = etapeNiveau("B1", extraDone))) return e;
    if (!examPasse("b1")) return "#/examen/b1";
    if ((e = etapeNiveau("B2", extraDone))) return e;
    if (!examPasse("b2")) return "#/examen/b2";
    if (!examPasse("finalb")) return "#/examen/finalb";
    if ((e = etapeNiveau("C1", extraDone))) return e;
    if (!examPasse("c1")) return "#/examen/c1";
    if ((e = etapeNiveau("C2", extraDone))) return e;
    if (!examPasse("c2")) return "#/examen/c2";
    if (!examPasse("finalc")) return "#/examen/finalc";
    return "#/";
  }
  /* Prochaine étape gardée du parcours (leçon, dictée ou examen) → hash */
  function prochaineEtape() { return prochaineEtapeCore(null); }
  /* Étape après une leçon : on considère la leçon courante comme faite, puis on
     renvoie la prochaine étape (leçon suivante, DICTÉE du module, ou examen). */
  function etapeApresLecon(idx) { return prochaineEtapeCore(flat[idx].lecon.id); }
  ...
  function renderHome() {
    ...
    const badge = niveauActuel
      ? '<div class="niveau-actuel">🎯 Ton niveau actuel : <strong>' + niveauActuel + "</strong> <span>" + (NIVEAU_LABELS[niveauActuel] || "") + "</span></div>"
      : "";
    ...
    // Bouton « changer de niveau » retiré : le parcours démarre toujours à A1.
    ...
    function renderNiveau(niv) {
      ...
      // Verrou d'examen : leçons faites + prérequis + DICTÉES du niveau réussies (obligatoires).
      const dicteesOk = dicteesFaites(niv.code);
      const unlocked = niveauTermine(niv.code) && prevPassed && dicteesOk;
      const lockText = !prevPassed
        ? ("Réussissez d'abord " + niv.prevLabel + ".")
        : !niveauTermine(niv.code)
          ? ("Terminez les leçons du niveau " + niv.code + " pour passer l'examen.")
          : ("✍️ Réussissez d'abord la/les dictée(s) du niveau " + niv.code + " (obligatoire) pour passer l'examen.");
  ...
  function renderLecon(id) {
    ...
    const suiteDictee = /\/dictee\//.test(suite);
    const suiteLabel = suiteExamen ? "🎓 Examen du niveau " + flat[idx].lecon.niveau + " →"
      : suiteDictee ? "✍️ Dictée du module (obligatoire) →"
      : (flat[idx + 1] ? flat[idx + 1].lecon.titre + " →" : "Continuer →");
    ...
    unlockNext = function () {
      ...
      next.href = suite;
      if (window.TG) window.TG.setMainButton(suiteExamen ? "🎓 Examen " + flat[idx].lecon.niveau : suiteDictee ? "✍️ Dictée du module" : "Suivant →", () => { location.hash = suite; });
    };
  ...
  function renderOnboarding() {
    // Choix de niveau DÉSACTIVÉ : tout le monde démarre au niveau A1 et progresse
    // ensuite vers le haut (les niveaux supérieurs se débloquent via les examens).
    if (!window.Progress.getNiveau()) placerAuNiveau("A1");
    var __dest = prochaineEtape();
    location.hash = (!__dest || __dest === "#/") ? "#/menu" : __dest;
    return;
    /* --- Ancien écran de choix de niveau (désactivé, conservé pour référence) --- */
  ...
  function renderDictee(moduleId) {
    ...
    corrige.addEventListener("click", () => {
      ...
      result.innerHTML =
        ...
        '<div class="rev-actions"><button class="btn btn-ghost" id="dictee-retry">↺ Recommencer</button>' +
        (reussi
          ? '<a class="btn btn-primary" href="' + prochaineEtape() + '">Continuer →</a>'
          : '<a class="btn btn-primary" href="#/">Retour au programme</a>') +
        '</div>' +
        (reussi ? '' : '<p class="dictee-feedback">✍️ Il faut <strong>70 %</strong> pour valider la dictée (obligatoire avant de continuer). Réécoute et retente — tu vas y arriver ! 💪</p>');
      result.classList.remove("hidden");
  ...
  function route() {
    ...
    // Choix de niveau / test de placement désactivés : on redirige vers le parcours A1.
    else if (hash.match(/^#\/placement-niveau\//)) renderOnboarding();
    else if (hash.match(/^#\/placement/)) renderOnboarding();
